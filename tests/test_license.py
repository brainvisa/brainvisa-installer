...
def test_license_License():
	t = TagLicense(	name = "MyLicense", 
					file_ = 'Licence_CeCILL_V2.1_en_EN.txt', 
					version = '3.21', 
					id_ = "MyID")
	l = License(t)
	l.create('out')
	...
	os.path.isfile('out/brainvisa.app.licenses.mylicense/meta/Licence_CeCILL_V2.1_en_EN.txt')
	x = XmlFile()
	...
	assert x.root.find('ReleaseDate').text == datetime.datetime.now().strftime("%Y-%m-%d")
	assert x.root.find('Virtual').text == 'true'
	assert x.root.find('Licenses')[0].attrib['file'] == 'Licence_CeCILL_V2.1_en_EN.txt'
