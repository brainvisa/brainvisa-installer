...
def test_Project__create_subcategorie():
	...
	assert os.path.isdir('%s/out/brainvisa.app.anatomist.usrdoc/meta' % FULLPATH)
	...
	assert os.path.isdir('%s/out/brainvisa.dev.anatomist.devdoc/meta' % FULLPATH)
	assert os.path.isfile('%s/out/brainvisa.app.anatomist.run/meta/package.xml' % FULLPATH)
	assert os.path.isfile('%s/out/brainvisa.app.anatomist.usrdoc/meta/package.xml' % FULLPATH)
	assert os.path.isfile('%s/out/brainvisa.dev.anatomist.dev/meta/package.xml' % FULLPATH)
	assert os.path.isfile('%s/out/brainvisa.dev.anatomist.devdoc/meta/package.xml' % FULLPATH)
	
	filename = '%s/out/brainvisa.app.anatomist.run/meta/package.xml' % FULLPATH
	assert '<DisplayName>%s</DisplayName>' % x.category_by_id('run').Name in open(filename, 'r').read()
	assert '<Name>brainvisa.app.anatomist.run</Name>' in open(filename, 'r').read()
...
def test_Project_create():
	...
	assert os.path.isfile('%s/out/brainvisa.app.axon/meta/package.xml' % FULLPATH)
	...
	assert os.path.isdir('%s/out/brainvisa.dev.axon.devdoc.axon_devdoc' % FULLPATH)
	assert os.path.isdir('%s/out/brainvisa.app.soma.run.soma_qtgui' % FULLPATH)

	assert os.path.isfile('%s/out/brainvisa.app.aims.run.aims_free/data/bin/AimsAttributedViewer' % FULLPATH)
