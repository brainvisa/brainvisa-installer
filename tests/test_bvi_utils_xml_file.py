...
def test_XmlFile_add_element():
    ...
    x.add_element('NewElement2', '0x3FFDD2S',
                  x.root_subelement('RemoteRepositories'))
    x.save(OUTPUT)
    del x
    x = XmlFile()
    x.read(OUTPUT)
    assert x.root_subelement('NewElement1').text == '0x39DD33S'
    assert x.root_subelement('RemoteRepositories').find(
        'NewElement2').text == '0x3FFDD2S'


def test_XmlFile_element_attribute_value():
    x = XmlFile()
    x.read(EXAMPLE)
    assert x.element_attribute_value(
        'Version', 'inheritVersion') == 'brainvisa.app'
    assert x.element_attribute_value(
        'Repository', 'testAttribute', x.root_subelement('RemoteRepositories')) == '02Xsd2'
...
def test_XmlFile_set_element_attribute_value():
    ...
    x.read(OUTPUT)
    assert x.element_attribute_value(
        'Watermark', 'testAttribute2') == '0x223FZE2'
