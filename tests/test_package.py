...
def test_create_2():
    ...
    x.create("%s/out" % FULLPATH)
    assert os.path.isdir(
        "%s/out/brainvisa.app.thirdparty.liblapack3gf_0" % FULLPATH)
    assert os.path.isdir(
        "%s/out/brainvisa.app.thirdparty.liblapack3gf_0/meta" % FULLPATH)
    assert os.path.isdir(
        "%s/out/brainvisa.app.thirdparty.liblapack3gf_0/data" % FULLPATH)
    assert os.path.isfile(
        "%s/out/brainvisa.app.thirdparty.liblapack3gf_0/meta/package.xml" % FULLPATH)
    assert x.version == '3.0'
    assert "<Version>3.0</Version>" in open(
        "%s/out/brainvisa.app.thirdparty.liblapack3gf_0/meta/package.xml" % FULLPATH, 'r').read()
...
def test_Package_create():
    ...
    x.create("%s/out" % FULLPATH)
    assert os.path.isdir(
        '%s/out/brainvisa.app.soma.usrdoc.soma_base_usrdoc' % FULLPATH)
    assert os.path.isdir(
        '%s/out/brainvisa.app.soma.usrdoc.soma_base_usrdoc/meta' % FULLPATH)
    assert os.path.isdir(
        '%s/out/brainvisa.app.soma.usrdoc.soma_base_usrdoc/data' % FULLPATH)
    assert os.path.isfile(
        '%s/out/brainvisa.app.soma.usrdoc.soma_base_usrdoc/meta/package.xml' % FULLPATH)
    y = XmlFile()
    ...
    assert y.root.find(
        'Name').text == 'brainvisa.app.soma.usrdoc.soma_base_usrdoc'
...
def test_Package_create_with_dependencies():
    ...
    assert os.path.isfile(
        "%s/out/brainvisa.app.axon.run.axon/meta/package.xml" % FULLPATH)
    assert 'brainvisa.app.thirdparty.bv_env' in open(
        "%s/out/brainvisa.app.axon.run.axon/meta/package.xml" % FULLPATH, 'r').read()
